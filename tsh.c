...
int nextjid = 1;            /* next job ID to allocate HOW TO MOVE??? */
 ...
 * each child process(OR EACH JOB) must have a unique process group ID so that our
...
void eval(char *cmdline) 
{
    ...
    char* argv[MAXARGS];
    int bf=parseline(cmdline,(char**) argv);
    if(argv[0]==NULL)
        return;
